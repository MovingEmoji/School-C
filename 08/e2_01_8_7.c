	...
	while(1){
		...
		while(1){
			...
			if(strcmp(rbuf, "hello") == 0) {
				strcpy(sbuf, "Hello master");
			} else if(strcmp(rbuf, "weather") == 0) {
				strcpy(sbuf, "Rainy today");
			} else if(strcmp(rbuf, "Are you siri?") == 0) {
				strcpy(sbuf, "No, I'm a stupid bot");
			} else if(strcmp(rbuf, "Siri, make a call") == 0) {
				strcpy(sbuf, "I can't");
			} else {
				strcpy(sbuf, "Unknown");
			}
...
Server
Server Start
accepted connection from 127.0.0.1, port=63097
recv_msg:hello
recv_msg:weather
